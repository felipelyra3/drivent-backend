...
describe("GET /hotels/vacancies/:hotelId", () => {
  it("should respond with status 401 if no token is given", async () => {
    const response = await server.get("/hotels/vacancies/1");
  ...
  it("should respond with status 401 if given token is not valid", async () => {
    ...
    const response = await server.get("/hotels/vacancies/1").set("Authorization", `Bearer ${token}`);
  ...
  describe("when token is valid", () => {
    it("should respond with status 400 when hotel is not valid", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
    ...
    it("should respond with status 202 and unsubscribe from activity", async () => {
      ...
      const response = await server.get(`/hotels/vacancies/${createdHotel.id}`).set("Authorization", `Bearer ${token}`);
