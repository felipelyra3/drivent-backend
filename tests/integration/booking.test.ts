...
describe("GET /booking/:roomId", () => {
  ...
  describe("when token is valid", () => {
    it("should respond with status 400 when roomId is not valid ", async () => {
      ...
      const response = await server.get("/booking/notvalid").set("Authorization", `Bearer ${token}`);

      expect(response.status).toEqual(httpStatus.BAD_REQUEST);
    });

    it("should respond with status 404 when there is no room with this id ", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
    ...
    it("should respond with status 200 when room exists ", async () => {
      const user = await createUser();
